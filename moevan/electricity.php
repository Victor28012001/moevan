...
if (strlen($_SESSION['id'] == 0)) {
    ...
    <head>
        ...
        <title>Electricity | Moevan</title>
            ...
            <!-- <style>
                ...
                <script>
                    function confirm() {
                        ...
                        icon: 'warning',
                        title: 'Checking',
                        text: 'Are you sure you want to carry out this transaction.',
                        background: '#1E1E1E',
    ...
    if (isset($_POST['Topup'])) {
        ...
        if ($balance >= $amount) {
            ...
            if ($res['code'] == "000") {
                $phone = $_POST['recepient'];
                echo "<script>alert($meter)</script>";
                $icon = addslashes('moevan icons\🦆 icon _Electricity_.svg');
                $amount = $_POST['amount'];
                ...
                $sql = "INSERT INTO transactions(fname,lname,ttype,email,tid,amount,dated,astatus,numbers,plan,plan_no) VALUES('$fname','$lname','$product','$email','$particulars','$amount','$dated','$status','$phone','electricity', '$meter')";
                $result1 = mysqli_query($GLOBALS['con'], $sql);
                echo "<script>alert($phone)</script>";
                $sql1 = "INSERT INTO inf(icon,messages,active) VALUES('$icon','Your electricity payment was successful.','1')";
                ...
                $result1 = mysqli_query($GLOBALS['con'], $sql);
                $sql1 = "INSERT INTO inf(icon,messages,active) VALUES('$icon','Your electricity payment was not successful.','1')";
                $result2 = mysqli_query($GLOBALS['con'], $sql1);
            }
