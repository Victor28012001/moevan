...
if (strlen($_SESSION['id'] == 0)) {
    ...
    <head>
        ...
        <title>Result Check | Moevan</title>
    ...
    if (isset($_POST['Topup'])) {
        ...
        if ($balance >= $amount) {
            ...
            if ($res['code'] == "000") {
                $icon = addslashes('moevan icons\moevandashboardicons\result checker.svg');
                $amount = $_POST['amount'];
                ...
                $sql = "INSERT INTO transactions(fname,lname,ttype,email,tid,amount,dated,astatus,numbers,plan,plan_no) VALUES('$fname','$lname','$product','$email','$particulars','$amount','$dated','$status','$phone','result','-')";
                $result1 = mysqli_query($GLOBALS['con'], $sql);
                $sql1 = "INSERT INTO inf(icon,messages,active) VALUES('$icon','Your result payment was successful.','1')";
                $result2 = mysqli_query($GLOBALS['con'], $sql1);
            } else {
                $icon = addslashes('moevan icons\moevandashboardicons\result checker.svg');
                $sql = "INSERT INTO transactions(fname,lname,ttype,email,tid,amount,dated,astatus) VALUES('$fname','$lname','Result','$email',0,'-',NOW(),'Error')";
                $result1 = mysqli_query($GLOBALS['con'], $sql);
                $sql1 = "INSERT INTO inf(icon,messages,active) VALUES('$icon','Your result payment was not successful.','1')";
                $result2 = mysqli_query($GLOBALS['con'], $sql1);
            }
